...
export const Geometry = {
    ...
    pathLength: (points: Vector2[]) => {
        let d = 0;
        for (let i = 1; i < points.length; i++) d += Geometry.distance(points[i - 1], points[i]);
        return d;
    },
    ...
    resample: (points: Vector2[], n: number) => {
        ...
        for (let i = 1; i < workingPoints.length; i++) {
            ...
            if (D + d >= interval) {
                const qx = workingPoints[i - 1].x + ((interval - D) / d) * (workingPoints[i].x - workingPoints[i - 1].x);
                const qy = workingPoints[i - 1].y + ((interval - D) / d) * (workingPoints[i].y - workingPoints[i - 1].y);
                const q = { x: qx, y: qy };
                newPoints.push(q);
        ...
        if (newPoints.length === n - 1) newPoints.push(workingPoints[workingPoints.length - 1]);
        return newPoints;
    },
