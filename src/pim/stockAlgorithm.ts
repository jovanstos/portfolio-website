...
function handleEarnings(currentStock: Stock): number {
  ...
  const recentHistory = currentStock.data.slice(
    -(EARNINGS_WEEK * DAYS_IN_WEEK),
  );
  const startPrice =
    recentHistory.length > 0 ? recentHistory[0][1] : currentStock.currentPrice;
  ...
  let beatProbability = 0.6 + stockGains;
  ...
  const volatilityHighEnd = Math.max(
    (currentStock.volatility + 3) / 1000,
    0.05,
  );
  ...
  const surprise =
    (actualEarnings - currentStock.projectedEarnings) /
    currentStock.projectedEarnings;
...
function getTrend(
  ...
  if (currentStock.companyNews != 0) {
    const amplifiedCompanyNews =
      currentStock.companyNews > 0
  ...
  if (currentStock.pOverE < 5) {
    ...
    randomNumberMax += 0.1; // Solid
  } else if (currentStock.pOverE < 45) {
    randomNumberMax += 0.2; // Growth Momentum
  } else if (currentStock.pOverE < 60) {
    randomNumberMax += 0.1; // Slowing down
  } else {
    randomNumberMax -= 0.2; // Overvalued/Bubble Correction
  }
...
function getChange(
  ...
  if (earningsSurprise !== null) {
    ...
    if (percentChange < 0.02 && currentStock.volatility > 0)
      percentChange = 0.02;
  }
  // Normal trading DAY
  else {
...
export function simulateNextWeek(
  ...
  const totalWeeklyChange =
    (currentStock.currentPrice - startingPrice) / startingPrice;
